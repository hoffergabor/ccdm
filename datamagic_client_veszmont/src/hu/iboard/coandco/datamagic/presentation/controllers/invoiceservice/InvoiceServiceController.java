...
public class InvoiceServiceController extends AbstractController implements Serializable {
	...
	public void invoiceDateChange(ActionEvent event) {
		 ...
		 */
		getData().setInvoice(
				getInvoiceService().getInvoices(getData().getVevokod(), getData().getFrom(), getData().getTo(), getData().getIn(), getData().getOut()));
		getData().setTempInvoices(getData().getInvoice());
		if (getData().getInvoice() != null && getData().getInvoice().size() >= 1000)
