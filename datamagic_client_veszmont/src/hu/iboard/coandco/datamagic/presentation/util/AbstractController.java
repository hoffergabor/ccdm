...
public abstract class AbstractController extends AbstractBaseDatabase {
	...
	protected LoginManager getLoginManager() {
		...
		LoginManager loginManager = (LoginManager) fc.getApplication().getELResolver().getValue(fc.getELContext(),
				null, "loginManager");
		return loginManager;
	}
	...
	protected Object addNewBeanForSession(String beanName) {
		return FacesContext.getCurrentInstance().getApplication().getExpressionFactory().createValueExpression(
				FacesContext.getCurrentInstance().getELContext(), "#{" + beanName + "}", Object.class).getValue(
	...
	protected String lastPage() {
		...
		FacesContext fc = FacesContext.getCurrentInstance();
		PreviewManager pvm = (PreviewManager) fc.getApplication().getELResolver().getValue(fc.getELContext(), null,
				PREVIEW_MANAGER);
