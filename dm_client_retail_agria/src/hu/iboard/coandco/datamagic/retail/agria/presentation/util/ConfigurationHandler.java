...
public final class ConfigurationHandler extends AbstractController {
	...
	private ConfigurationHandler() {
		try {
			...
			if (defaultArnevKod == null)
				defaultArnevKod = 0;
			
			valueString = doc.getElementsByTagName(Constants.CONFIG_ELEMENT_ADAPT_DISCOUNT).item(0).getFirstChild().getNodeValue();
			adaptDiscount = new Boolean(valueString);
			if (adaptDiscount == null)
				adaptDiscount = true;
			
			valueString = doc.getElementsByTagName(Constants.CONFIG_ELEMENT_NAME_CODE).item(0).getFirstChild().getNodeValue();
			megnevezesKod = new Integer(valueString);
			if (megnevezesKod == null)
				megnevezesKod = 0;			
			...
			for (Node childNode = doc.getElementsByTagName(Constants.CONFIG_ELEMENT_ORDER_PARAMETERS).item(0).getFirstChild(); childNode != null;) {
				...
				if (nodeName == CONFIG_ELEMENT_ORDER_CHARGE)
				...
				else if (nodeName == CONFIG_ELEMENT_MINIMUM_ORDER_VALUE) {
					minimumOrderValue = new Integer(childNode.getFirstChild().getNodeValue());
				}
				else if (nodeName == CONFIG_ELEMENT_CHARGE_ITEM_CODE) {
