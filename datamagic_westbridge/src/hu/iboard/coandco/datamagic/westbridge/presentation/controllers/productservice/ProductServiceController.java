package hu.iboard.coandco.datamagic.westbridge.presentation.controllers.productservice;
...
public class ProductServiceController extends AbstractController {
	...
	public String productSearchAction() {
		...
		Arnev arnev = getManagedPartner().getVevocsop().getArnev();

		getData().setProducts(
				getProductService().getProductForSearch(arnev, getData().getSku(), getData().getProductName()));
		return null;
	}
	...
	public void searchAdvanced(ActionEvent event) {
		...
		Arnev arnev = getManagedPartner().getVevocsop().getArnev();
		getData().setProducts(getProductService().getProductForAdvancedSearch(arnev, cikkszam + "%"));
