package hu.iboard.coandco.datamagic.westbridge.presentation.controllers.invoiceservice;
...
public class InvoiceServiceController extends AbstractController implements Serializable {
	...
	public void invoiceDateChange(ActionEvent event) {
		 ...
		 * ); return; } }
		 */
		List<InvoiceVO> invoices = getInvoiceService().getInvoices(getData().getVevokod(), getData().getFrom(), getData().getTo(), getData().getIn(),
				getData().getOut());
	...
	public String invoiceItemsAction() {
		...
		getData().setInvoiceItems(
				getInvoiceService().getInvoiceItemsByInvoiceId(getData().getSelectedInvoice().getSorszam(), getData().getSelectedInvoice().getKimeno()));
		return null;
	}
	...
	private List<Object[]> getInvoiceOutForChart() {
		...
		List<Object[]> newList = new ArrayList<Object[]>();
		listOut = getInvoiceService().getInvoiceOutForChart(getManagedUser(), getData().getVevokod(), new Timestamp(getData().getChartFrom().getTime()),
				new Timestamp(getData().getChartTo().getTime()));
