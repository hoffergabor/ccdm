...
public class PartnerDataUpdateController extends AbstractController {
	...
	public void loadData() {
		if (getManagedPartner() != null) {
			...
			getData().setPartner(getManagedStore());
		} else {
	...
	public void updatePartnerData(ActionEvent event) {
		...
		if (!checkPartnerData(getData().getPartner())) {
			addErrorMessage("", "Név és e-mail cím megadása kötelező!");
			return;
		}
		try {
			...
			if (partner == null) {
				...
				addErrorMessage("", "A partner adatainak frissítése közben hiba történt.");
				return;
			}
			logger.info(getData().getPartner().getNev() + " data is updated!");
			addInfoMessage("", "A partner adatainak frissítése megtörtént.");
		} catch (Exception e) {
	...
	public String changePasswordAction() {
		...
		try {
			if (getData().getPassword1().equals(getData().getPassword2())) {
				...
				addInfoMessage("", "A jelszó módosítása megtörtént.");
			}
		} catch (Exception e) {
			logger.error("Error by update user", e);
			addFatalMessage("", "A jelszó módosítása közben hiba történt!");
			getData().setPassword1("");
			getData().setPassword2("");
			return null;
		}
	...
	public void sendEmail(ActionEvent event) {
		...
		try {
			...
			boolean issuccess = getEmailService().sendEmail(EMAIL_SMTP_HOST, EMAIL_SMTP_USER, EMAIL_SMTP_PASS, EMAIL_FROM, "hoffer.gabor@coandco.cc",
					"Adatmódosítás", body);
			if (issuccess)
				addInfoMessage("", "Email küldés sikeres!");
			else {
				addErrorMessage("", "Email küldés sikeretelen!");
