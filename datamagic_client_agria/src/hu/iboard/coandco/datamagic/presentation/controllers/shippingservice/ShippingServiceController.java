...
public class ShippingServiceController extends AbstractController {
	...
	public void shippingDateChange(ActionEvent event) {
		 ...
		 * ); return; } }
		 */
		List<ShippingVO> shippings = new ArrayList<ShippingVO>();
		if (getManagedWorker() != null) {
			shippings = getShippingService().getShippingsByDateFiltered(null, new Date(getData().getFrom().getTime()), new Date(getData().getTo().getTime()));
		} else if (getManagedStore() != null) {
			shippings = getShippingService().getShippingsByDateFiltered(getManagedStore().getVevokod(), new Date(getData().getFrom().getTime()),
					new Date(getData().getTo().getTime()));
		} else if (getManagedPartner() != null && getManagedStores() != null && getManagedStores().size() > 0) {
			List<ShippingVO> temp = new ArrayList<ShippingVO>();
			for (Partner partner : getManagedStores()) {
				temp = getShippingService().getShippingsByDateFiltered(partner.getVevokod(), new Date(getData().getFrom().getTime()),
						new Date(getData().getTo().getTime()));
				if (temp != null && temp.size() > 0)
					shippings.addAll(temp);
			}

		}
		getData().setShippings(shippings);
		initSummary();
	}
	...
	private List<Object[]> getShippingsForChart() {
		...
		listIn = getShippingService().getShippingsForChart(getManagedUser(), getData().getVevokod(), new Timestamp(getData().getChartFrom().getTime()),
	...
	public void getShippingItem(ActionEvent event) {
		...
		List<ShippingItemVO> items = getShippingService().getShippingItemsByShippingId(getData().getSelectedShipping().getSorszam());
		if (items == null) {
			getData().setShippingItems(new ArrayList<ShippingItemVO>());
			return;
		}
	...
	public String submitChooseStore() {
		...
		getData().setShippings(
				getShippingService().getShippingsByDateFiltered(getData().getSelectedPartnerId(), new Date(getData().getFrom().getTime()),
						new Date(getData().getTo().getTime())));
		initSummary();
		return null;
